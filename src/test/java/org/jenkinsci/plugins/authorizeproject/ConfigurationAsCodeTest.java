...
public class ConfigurationAsCodeTest {
    ...
    public void importProjectTriggeringUsersAuthorizationStrategy() {
        ...
        assertThat(authenticators, hasSize(1));
        assertThat(queueItemAuthenticator.getDisabledStrategies(), equalTo(new HashSet<>(Arrays.asList("systemAuthorizationStrategy", "specificUsersAuthorizationStrategy", "triggeringUsersAuthorizationStrategy"))));
        assertThat(queueItemAuthenticator.getEnabledStrategies(), equalTo(new HashSet<>(Arrays.asList("anonymousAuthorizationStrategy"))));
    }
    ...
    private void assertExport(String resourcePath) throws Exception {
        ...
        CNode queueItemAuthenticator = getSecurityRoot(context).get("queueItemAuthenticator");

        String exported = toYamlString(queueItemAuthenticator);
        String expected = toStringFromYamlFile(this, resourcePath);
